# Mapa de `src`

## Papel da pasta
`src` contém o código-fonte ativo do frontend. Tudo que define comportamento, layout, navegação, estado e integração com a API parte daqui.

## Estrutura principal
- `App.vue`: raiz visual da aplicação.
- `main.ts`: bootstrap do app, plugins e registro global.
- `assets/`: CSS principal e estilos compartilhados.
- `components/`: componentes reutilizáveis, incluindo primitives de UI, modais compartilhados e padrões mobile como bottombar fixa.
- `composables/`: lógica reutilizável de interface e fluxo.
- `hooks/`: helpers menores e verificações isoladas.
- `layouts/`: shells de navegação e estrutura de página.
- `lib/`: helpers utilitários de baixo nível.
- `pages/`: telas por domínio.
- `pluguins/`: integrações como socket.
- `repositories/`: acesso à API, incluindo `assinatura-repository.ts` para o domínio recorrente `/assinaturas` e `whatsapp-repository.ts` para o atendimento WhatsApp.
- `pages/loja/`: gestão interna de pedidos/personalização e vitrine pública por slug; `composables/useStoreCart.ts` persiste o carrinho isolado por loja e `repositories/loja-repository.ts` concentra os contratos públicos e privados.
- `pages/site/`: site institucional público; `siteNiches.ts` centraliza os seis segmentos destacados e abastece a vitrine da home e as páginas dinâmicas em `/site/solucoes/:nicho`.
- `router/`: definição de rotas.
- `stores/`: estado global e por domínio.
- `types/`: contratos TypeScript consumidos pelo app.
- `utils/`: infraestrutura transversal.
- URLs de arquivos renderizáveis no frontend devem passar por `utils/fileUrl.ts`, para aceitar tanto caminhos locais servidos pelo backend quanto URLs públicas absolutas retornadas pelo storage remoto.
- `__tests__/`: testes do frontend.

## Fluxo recomendado
- Interface de tela nasce em `pages`.
- Estado e ações compartilhadas do módulo vão para `stores`.
- Requisições externas vão para `repositories`.
- Modais e páginas de detalhe com dados vivos, como OS, produto e comprovantes operacionais do PDV, devem carregar o estado via store/repository e recarregar após mutações relevantes.
- No domínio de produtos, a tela de detalhe deve separar a variante selecionada, a lista de variantes e a visão geral consolidada do produto base para reduzir ambiguidade para o usuário final.
- Os formulários de produto base e variante reutilizam `pages/produtos/formulario/CalculadoraPrecificacao.vue` para calcular o preço sugerido a partir de compra, produção, custos adicionais e lucro percentual sobre o custo; os custos adicionais são auxiliares da sessão e apenas compra, produção e preço sugerido são aplicados ao formulário.
- Dashboards por domínio devem preferir agregações do backend via repository, incluindo filtros por período e isolamento por `contaId` do usuário autenticado.
- Em reservas, `pages/reservas/Painel.vue` consome a agregação de `repositories/reservas-gerais-repository.ts`; a rota `/reservas/painel` é o dashboard e `/reservas/lista` continua sendo a tela operacional. O atalho “Nova reserva” abre o cadastro existente na listagem por query string, sem duplicar o formulário.
- Em serviços, `pages/servicos/Dashboard.vue` consome `OrdensServicoRepository.getPainel` e segue o mesmo padrão executivo dos painéis de Vendas e Produtos: atalhos de período, comparação com o período anterior, KPIs, pipeline operacional, gráficos, rankings, fila de atenção e calendário interativo. Ações e detalhes continuam reutilizando o store e os modais de Ordens de Serviço.
- A dashboard principal deve atuar como central executiva do sistema, reunindo KPIs, alertas operacionais, atalhos e resumos por domínio sem quebrar o padrão visual já usado nas demais páginas.
- Módulos com produto base e variantes devem explicitar quando a ação cria um item novo do zero ou apenas uma variante adicional, e a listagem pode alternar entre visão agregada por base e visão analítica por variante.
- A listagem principal de `pages/produtos/Home.vue` deve usar modal de filtros no padrão do projeto, com `Select2Ajax` para status, categoria, estoque baixo e mais vendas, enquanto o dashboard de produtos preserva o recorte por período usando apenas o calendário nativo, sem atalhos rápidos duplicados fora do componente.
- Fluxos de relatório do domínio devem aceitar contexto pré-selecionado (produto base ou variante) e filtros opcionais de período, em vez de exigir reescolha manual do item a cada exportação.
- Listagens mobile que substituem tabelas desktop devem preferir cards compactos, ações rápidas e bottombar fixa para busca, paginação ou atalhos do módulo.
- No domínio financeiro, contas financeiras e categorias seguem como fluxos auxiliares de configuração dentro de `financeiro/lancamentos`, enquanto as jornadas operacionais podem ser separadas em `lancamentos`, `acompanhamento`, `contas a receber`, `contas a pagar` e `assinaturas a pagar` para reduzir atrito de uso.
- `financeiro/assinaturas-pagar` deve tratar despesas recorrentes operacionais como um domínio próprio dentro do financeiro, com cadastro de serviço, links úteis, ícone, cor de destaque, geração opcional de lançamento vinculado e automação segura do próximo ciclo após baixa do lançamento atual.
- A listagem principal de `financeiro/lancamentos` agora deve tratar filtros avançados como contrato real de backend/frontend, usando período, tipo, status, origem, conta, categoria e cliente/fornecedor como filtros estruturados além da busca textual do `DataTable`.
- O filtro de `pages/vendas/formulario/ModalFiltro.vue` deve seguir o mesmo padrão e usar `Select2Ajax` para status, cliente, produto, serviço, vendedor e regra de desconto, preservando o contrato atual de período e a mesma combinação de filtros em desktop e mobile.
- A criação operacional de cobranças deve respeitar a flag `permitirCriacaoCobranca` carregada da conta: os botões precisam sumir da UI quando a conta desativar o recurso e o store/modal ainda deve bloquear abertura tardia para evitar inconsistência visual ou operacional.
- As barras inferiores mobile dessas telas operacionais devem reutilizar `components/mobile/MobileBottomBar.vue` para manter o mesmo padrão visual e de navegação usado na dashboard principal, em vendas e nas demais listagens móveis.
- O domínio técnico `assinaturas` aparece para o usuário como app **Contratos** e reutiliza esse mesmo repertório de UI para cadastro de planos e contratos, além de padronizar os selects de gateway e tipo de pagamento com o mesmo conjunto visível no gerador de cobrança do financeiro.
- As páginas `assinaturas/AssinaturasHome.vue` e `assinaturas/PlanosHome.vue` seguem o padrão maduro do projeto: `DataTable` no desktop, cards paginados no mobile e edição via modal no mesmo contexto da listagem, com seções colapsáveis inspiradas no cadastro de produtos.
- Os detalhes de assinatura agora devem separar `Cobranças`, `Comodatos` e `Histórico` em tabs com o mesmo padrão visual e estrutural já usado em `produtos/others/DetalhesProduto.vue`, inclusive com scroll horizontal natural em telas menores.
- No domínio de assinaturas, a exclusão de plano só é permitida sem assinaturas vinculadas; a exclusão de assinatura bloqueia comodatos em uso, cobranças ativas e ciclos já pagos para preservar consistência operacional e financeira.
- Cobranças recorrentes desse domínio agora expõem geração direta no gateway com PIX, boleto e link de pagamento, mantendo no próprio ciclo a referência do gateway e o link retornado para uso posterior.
- Cancelamento e estorno no fluxo recorrente seguem as mesmas restrições do financeiro: cancelamento apenas para cobrança pendente e estorno apenas para cobrança efetivada, ambos somente em PIX/boleto quando o gateway suportar a operação.
- A exclusão manual da cobrança recorrente no ciclo agora bloqueia PIX e boleto até que a cobrança tenha sido cancelada/estornada, mas libera a remoção de links de pagamento quando o fluxo do gateway não oferece bloqueio automático.
- O reajuste de cobrança recorrente agora cancela a cobrança pendente anterior no gateway, atualiza o valor do ciclo e recria automaticamente a nova cobrança, preservando rastreabilidade no histórico da assinatura.
- A geração manual de ciclo recorrente exige confirmação explícita em modal antes da execução, com loading e prevenção de clique duplicado durante o envio.
- A App Store e a sidebar principal agora tratam `assinaturas` (exibido como **Contratos**), `core-ia`, `whatsapp` e `atendimento` como apps pagos por conta, além dos apps gratuitos `mercado-pago` e `abacatepay`: a Store organiza a vitrine em categorias com cards compactos de catálogo, um resumo horizontal mais enxuto no topo, o menu só aparece quando o módulo está ativo e a seção `Apps` da sidebar só é renderizada quando existe pelo menos um app visível na conta; o superadmin pode alternar esses apps em `pages/admin/assinantes/ModalGerenciarAssinante.vue`.
- No modal de gerenciamento do assinante, o bloco “Recuperar acesso do cliente” oferece duas ações separadas: enviar ao e-mail do primeiro usuário root um link de recuperação válido por 30 minutos ou redefinir manualmente a senha root. O envio exige confirmação, possui loading próprio e usa `ContaRepository.enviarRecuperacaoSenhaRootAdmin`.
- As credenciais operacionais de Mercado Pago e AbacatePay saíram de `configs/ConfiguracaoPage.vue` e passaram a ser configuradas diretamente nos modais dos apps correspondentes em `pages/store/StorePage.vue`, mantendo o mesmo contrato de `ParametrosConta` no backend.
- A tela `configs/ConfiguracaoPage.vue` voltou a concentrar apenas preferências operacionais gerais, notificações, impressão e parâmetros financeiros que não pertencem à App Store.
- O gateway da mensalidade SaaS agora é controlado pelo superadmin em `pages/admin/configuracoes/Home.vue`; a renovação continua abrindo checkout hospedado, mas o backend passa a escolher o provedor a partir de `Contas.gateway` sincronizado globalmente.
- O modo CEO gerencia avisos operacionais em `pages/admin/informativos/Home.vue` (`/admin/informativos`), com público global, por app ou por contas. No cliente, `components/layout/InformativosStatusButton.vue` aparece ao lado do tema no header e no rodapé da sidebar mobile, usa `stores/informativos/useInformativos.ts`, registra leitura/dispensa e reage a `informativos:updated` sem abrir modal, toast ou banner automaticamente.
- A página `pages/whatsapp/WhatsAppPage.vue` (app `whatsapp`, rota `/whatsapp`) concentra apenas a gestão de instâncias W-API por conta: conexão por QR Code/pairing code, ações da instância e prévia/sincronização confirmada de webhooks por instância. Consome eventos Socket.IO `whatsapp:instancia:updated`.
- A página `pages/atendimento/AtendimentoPage.vue` (app `atendimento`, rota `/atendimento`) é a central de conversas em tempo real: inbox estilo chat, envio de texto/mídias, filas/setor, finalizar/reabrir e vínculo manual ou automático com clientes. Reaproveita o mesmo backend de conversas do WhatsApp via `repositories/whatsapp-repository.ts` e consome os eventos Socket.IO `whatsapp:conversa:updated` e `whatsapp:mensagem:created`. Também permite iniciar uma conversa a partir de um cliente pelo botão "Nova" (diálogo com busca de cliente + seleção de instância) ou automaticamente quando aberta com `?cliente=<id>` — usado pelo botão "Iniciar atendimento" em `pages/clientes/DetalhesCliente.vue`, visível apenas quando o app `atendimento` está ativo e o cliente tem telefone/WhatsApp. Cada instância W-API funciona como uma caixa de atendimento: quando há mais de uma instância, um seletor de caixas no topo permite alternar entre elas (ou "Todas"), filtrando as conversas por `instanciaId` via `GET /whatsapp/conversas?instanciaId=`. As abas seguem o fluxo Espera (PENDENTE) → Abertas (ABERTA) → Finalizadas (FINALIZADA), com a aba inicial sempre em "Abertas"; responder só é liberado após assumir o atendimento pelo botão "Atender" (`POST /whatsapp/conversas/:id/atender`), que registra o atendente. O menu lateral `atendimento` é um pai com submenus `atendimento:painel`, `atendimento:chat` (a tela de chat, rota `/atendimento`), `atendimento:agentes` e `atendimento:relatorios` (páginas placeholder em `pages/atendimento/`), todos ocultáveis em Configurações. O composer tem ações rápidas (ferramentas para enviar dados do sistema ao cliente), abertas pelo ícone Blocks ou digitando `/`: a primeira é a ferramenta **Venda**, que só fica disponível quando o contato está vinculado a um cliente e lista/seleciona apenas vendas desse cliente (`GET`/`POST /whatsapp/conversas/:id/ferramentas/vendas`).
- O realtime do atendimento é best-effort: o cliente reentra na sala após reconectar e reconcilia conversas/mensagens com a API a cada 10 segundos enquanto a página está visível, garantindo recuperação quando um broadcast é perdido.
- Quando a mensalidade do SaaS usa AbacatePay, a experiência final de pagamento oferece PIX e cartão no mesmo link de renovação, sem expor chaves da plataforma na UI da conta.
- Nas cobranças internas do ERP, Mercado Pago e AbacatePay usam credenciais salvas em `ParametrosConta`, preservando a separação entre tenant e plataforma; no caso da AbacatePay, o backend ainda tenta sincronizar a webhook da conta automaticamente quando a `BASE_URL` é HTTPS.
- Telas financeiras de acompanhamento, contas a pagar e contas a receber devem expor filtros operacionais claros, criação rápida de lançamento, abas compactas para lista/KPI/calendário e cálculos de saldo, pendências e atrasos sempre baseados nas parcelas do lançamento, não apenas no cabeçalho.
- A listagem de contas financeiras e o painel financeiro devem mostrar o saldo atual calculado por movimentação e reagir a eventos de socket do domínio financeiro para manter os números sincronizados após lançamentos, ajustes, transferências e liquidações por webhook.
- Contas financeiras agora podem carregar cor de destaque e ícone opcional, reaproveitando o padrão visual já usado em `financeiro/assinaturas-pagar` e o mesmo pipeline de upload público do backend.
- O domínio de ordens de serviço agora também deve tratar faturamento e geração de cobrança como fluxos operacionais de primeira classe, reaproveitando os modais e stores do financeiro quando houver aderência em vez de criar uma UX paralela.
- Ordens de serviço faturadas não devem abrir fluxo de edição no frontend; a UI precisa bloquear essa entrada antes do usuário entrar em um formulário inválido.
- Componentes de tabs devem permitir scroll horizontal nativo em telas menores quando a quantidade de abas ultrapassar a largura disponível.
- Comportamento transversal ou reutilizável vai para `composables`.
- Infraestrutura e detalhes técnicos ficam em `utils` ou `lib`.
- A listagem CEO em `pages/admin/assinantes/Home.vue` e sua tabela exibem usuários online e o último login agregado de cada conta. Desktop e mobile atualizam a atividade a cada 30 segundos; `repositories/conta-repository.ts` tipa `usuariosOnline`, `temUsuarioOnline` e `ultimoLoginEm`.

## Cuidados
- Evitar lógica de regra de negócio grande direto em componentes visuais.
- Evitar chamadas `axios` direto em páginas, exceto manutenção pontual muito localizada.
- Ao criar funcionalidade nova, seguir o domínio mais próximo já existente em `pages`.
- Evitar estado fake/local para fluxos que dependem de API; em detalhes operacionais, preferir loading explícito, reload e feedback visual coerente.
