...
const dataMobile = ref<AssinaturaCicloListItem[]>([])
...
async function loadMobile(page = 1) {
  loading.value = true
  try {
    const response = await http.get('/assinaturas/cobrancas/mobile', {
      params: {
        search: searchQuery.value,
        status: store.cobrancasFilters.status,
        limit: 10,
        page,
      },
    })
...
<template>
  <div class="mt-2 flex max-h-[calc(100vh-13rem)] flex-col gap-2 overflow-auto md:max-h-full">
    ...
    <div v-else class="flex flex-col gap-2 pb-20">
      ...
      >
        <div class="text-center">
          ...
          <p class="text-gray-500 dark:text-gray-300">Nenhuma cobrança encontrada.</p>
        </div>
      </div>
      ...
      >
        <div class="flex items-start justify-between gap-3">
          <div>
            <div class="text-sm font-semibold dark:text-white">{{ row.assinatura.nomeContrato }}</div>
            <div class="text-xs text-gray-500 dark:text-gray-400">{{ row.assinatura.cliente }} • ref. {{ row.referencia }}</div>
          </div>
          <div class="text-xs" :class="getStatusCicloMeta(row.status).className">
            {{ getStatusCicloMeta(row.status).label }}
          </div>
        </div>

        <div class="mt-2 text-sm text-gray-700 dark:text-gray-200">
          {{ formatCurrencyBR(row.valorCobrado) }} • {{ formatDateToPtBR(row.inicioPeriodo) }} → {{ formatDateToPtBR(row.fimPeriodo) }}
        </div>
    ...
    <ModalView v-model:open="showSearchModal" title="Buscar cobranças" description="Refine por contrato, cliente ou UID.">
