...
- `restaurante/Cardapio.vue` usa cards compactos responsivos e modais shadcn para vincular produtos existentes, associar grupos e editar sabores/complementos; preço base e estoque continuam pertencendo ao cadastro de produtos.
- `restaurante/Salao.vue` usa cards compactos por status para abrir sessões e comandas, lançar itens e solicitar a conta. O carregamento operacional descarta grupos inativos e recebe as opções ativas do catálogo privado; erros de roteamento KDS retornados pela API são exibidos no toast sem fechar o modal. `Pedidos.vue` adota a mesma densidade para a fila operacional; `Kds.vue` usa tickets por ponto, atualização via Socket.IO e transições pendente → preparando → pronto → entregue.
- `restaurante/Impressao.vue` administra estações e regras por ponto e pode operar como cliente local: conecta ao QZ Tray, consulta trabalhos por polling/evento, deduplica UIDs no navegador e confirma sucesso ou falha ao backend.
...
- O painel de Serviços em `servicos/Dashboard.vue` deve manter filtros de período reais e apresentar valor das ordens, faturamento, ticket médio, volume, pipeline, clientes, catálogo, evolução diária, distribuição por status, serviços/técnicos em destaque e ordens que pedem atenção. O calendário compartilhado permanece disponível no mesmo contexto e abre detalhes ou uma nova OS pelo horário selecionado.
