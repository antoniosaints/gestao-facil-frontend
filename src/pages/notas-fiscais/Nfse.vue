...
import { Download, FileText, LoaderCircle, Plus, Settings2, XCircle } from 'lucide-vue-next'
...
async function emit() {
  ...
  try {
    ...
    toast.success(isD2ti.value ? (invoice.status === 'HOMOLOGADA' ? 'XML validado em homologação.' : 'NFS-e autorizada pela prefeitura.') : 'NFS-e autorizada pela Geranet.')
  } catch (error: any) {
...
async function cancel(invoice: NfseListItem) {
  const motivo = window.prompt('Justificativa do cancelamento da NFS-e:')
  if (!motivo) return
  try { issuing.value = true; await NotasFiscaisRepository.cancelDocument(invoice.id, motivo); toast.success('Cancelamento concluído.'); await load() }
  catch (error: any) { toast.error(error?.response?.data?.error?.message || 'Não foi possível cancelar a NFS-e.') }
  finally { issuing.value = false }
}
</script>
...
<template>
  <div class="mx-auto max-w-7xl space-y-5 pb-10">
    ...
    <template v-else>
      <Card :class="ready ? 'border-emerald-500/35' : 'border-amber-500/35'"><CardHeader><CardTitle>{{ ready ? 'Emissor configurado' : 'Conclua a configuração fiscal' }}</CardTitle><CardDescription>{{ ready ? (isD2ti ? 'Integração D2TI de São Mateus do Maranhão pronta para homologação ou produção.' : 'Geranet selecionada: a NFS-e usa o A1 do assinante, os códigos de serviço e o município IBGE.') : 'Preencha os dados fiscais, parâmetros da atividade e a credencial do emissor.' }}</CardDescription></CardHeader><CardContent><Button v-if="!ready" @click="router.push({ name: 'notas-fiscais-configuracoes' })"><Settings2 />Abrir configuração</Button></CardContent></Card>

      <Card v-if="ready" class="border-primary/25">
        <CardHeader><CardTitle class="flex items-center gap-2"><Plus class="size-5 text-primary" />Emitir NFS-e</CardTitle><CardDescription>{{ isD2ti ? 'A emissão é síncrona. Em caso de falha de conexão, a tentativa fica bloqueada para evitar uma nota duplicada no município.' : 'A Geranet assina e transmite usando o A1 do assinante. Em caso de conexão incerta, o sistema bloqueia o reenvio automático.' }}</CardDescription></CardHeader>
        <CardContent class="grid gap-4 sm:grid-cols-2">
          ...
          <div class="sm:col-span-2"><Button :disabled="issuing" @click="emit"><LoaderCircle v-if="issuing" class="animate-spin" /><Plus v-else />Emitir NFS-e</Button></div>
        </CardContent>
      </Card>


      <Card><CardHeader><CardTitle>Histórico de NFS-e</CardTitle><CardDescription>Documentos emitidos por esta conta.</CardDescription></CardHeader><CardContent><div v-if="!invoices.length" class="flex min-h-40 flex-col items-center justify-center gap-2 rounded-xl border border-dashed text-center text-sm text-muted-foreground"><FileText class="size-7" />Nenhuma NFS-e emitida ainda.</div><div v-else class="divide-y rounded-xl border"><div v-for="invoice in invoices" :key="invoice.id" class="flex flex-col gap-2 p-4 sm:flex-row sm:items-center sm:justify-between"><div><p class="font-semibold">{{ invoice.cliente?.nome || 'Consumidor final' }}</p><p class="text-xs text-muted-foreground">{{ invoice.numero ? `NFS-e ${invoice.numero}` : 'Emissão sem número' }} · {{ new Date(invoice.criadoEm).toLocaleString('pt-BR') }}</p></div><div class="flex flex-wrap items-center gap-2"><strong>R$ {{ invoice.valorTotal.toFixed(2).replace('.', ',') }}</strong><Badge variant="secondary">{{ statusLabel[invoice.status] || invoice.status }}</Badge><Button v-if="invoice.status === 'AUTORIZADA'" size="sm" variant="outline" @click="NotasFiscaisRepository.downloadDocument(invoice.id, 'pdf', `NFSE-${invoice.numero || invoice.id}.pdf`)"><Download />PDF</Button><Button v-if="invoice.status === 'AUTORIZADA'" size="sm" variant="outline" @click="NotasFiscaisRepository.downloadDocument(invoice.id, 'xml', `NFSE-${invoice.numero || invoice.id}.xml`)"><Download />XML</Button><Button v-if="invoice.status === 'AUTORIZADA'" size="sm" variant="destructive" :disabled="issuing" @click="cancel(invoice)"><XCircle />Cancelar</Button></div></div></div></CardContent></Card>
    </template>
  </div>
</template>
