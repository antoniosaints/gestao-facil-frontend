...
import { ExternalLink, FileText, LoaderCircle, Plus, Settings2, ShieldAlert } from 'lucide-vue-next'
...
const isD2ti = computed(() => config.value?.integracao.tipo === 'TOKEN_D2TI')
...
async function emit() {
  if (!form.clienteId || form.valorTotal <= 0 || !form.codigoMunicipioTomador || form.discriminacao.trim().length < 3) {
    toast.info('Selecione o tomador, informe valor, código TOM do município e a descrição do serviço.')
    return
  }
  try {
    issuing.value = true
    const invoice = await NotasFiscaisRepository.emitD2ti({ ...form, clienteId: form.clienteId }, crypto.randomUUID())
    ...
    toast.success(invoice.status === 'HOMOLOGADA' ? 'XML validado em homologação.' : 'NFS-e autorizada pela prefeitura.')
  } catch (error: any) {
...
<template>
  <div class="mx-auto max-w-6xl space-y-5 pb-10">
    ...
    <template v-else>
      <Card :class="ready ? 'border-emerald-500/35' : 'border-amber-500/35'"><CardHeader><CardTitle>{{ ready ? 'Emissor configurado' : 'Conclua a configuração fiscal' }}</CardTitle><CardDescription>{{ ready ? (isD2ti ? 'Integração D2TI de São Mateus do Maranhão pronta para homologação ou produção.' : 'A emissão automática ainda depende da integração do município.') : 'Preencha os dados fiscais, parâmetros da atividade e a credencial do emissor.' }}</CardDescription></CardHeader><CardContent><Button v-if="!ready" @click="router.push({ name: 'notas-fiscais-configuracoes' })"><Settings2 />Abrir configuração</Button></CardContent></Card>

      <Card v-if="ready && isD2ti" class="border-primary/25">
        <CardHeader><CardTitle class="flex items-center gap-2"><Plus class="size-5 text-primary" />Emitir NFS-e</CardTitle><CardDescription>A emissão é síncrona. Em caso de falha de conexão, a tentativa fica bloqueada para evitar uma nota duplicada no município.</CardDescription></CardHeader>
        <CardContent class="grid gap-4 sm:grid-cols-2">
          ...
          <div class="space-y-1.5"><Label for="nfse-codigo-tom">Código TOM do município do tomador</Label><Input id="nfse-codigo-tom" v-model="form.codigoMunicipioTomador" inputmode="numeric" placeholder="Ex.: 0923 para São Mateus" /></div>
      ...
      <Card v-else-if="ready"><CardContent class="flex items-center gap-2 py-5 text-sm text-muted-foreground"><ShieldAlert class="size-5 text-amber-500" />Este município ainda não possui uma integração automática configurada.</CardContent></Card>
