...
function pedidoStatusLabel(order: RestaurantePedido) {
...
<template>
  <main class="driver-app">
    <section v-if="!signedIn" class="driver-login">
      ...
      <p class="security">
        <ShieldCheck :size="15" />Seu acesso é protegido e vinculado ao restaurante.
      </p>
    </section>
    ...
    <template v-else>
      ...
      <section class="driver-content">
        <template v-if="view === 'history'">
          <button class="back-offers" @click="openOffers">
            <ChevronRight :size="17" />Voltar às ofertas
          </button>
          ...
          <div v-if="historyLoading && !history.length" class="loading">
            <RefreshCw class="animate-spin" />Carregando histórico...
          </div>
          <div v-else-if="!history.length" class="empty">
            <History :size="34" />
            <h2>Nenhuma entrega finalizada</h2>
          ...
          <template v-else-if="activeDelivery">
            ...
            <article class="active-card">
              <div class="active-top">
                <span class="order-code">{{ activeDelivery.codigo }}</span>
                <span class="status">{{ activeDelivery.entregaStatus.replace('_', ' ') }}</span>
              </div>
              ...
              <button class="issue" :disabled="acting" @click="reportIssue">
                <XCircle :size="16" />Reportar problema
              </button>
            </article>
          </template>
          <template v-else>
