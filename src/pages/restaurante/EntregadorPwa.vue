...
const view = ref<'offers' | 'history'>('offers')
const history = ref<RestaurantePedido[]>([])
...
function nextAction(order: RestaurantePedido) {
  ...
  return actions[order.entregaStatus]
}
...
async function loadHistory(page = 1) {
  ...
  try {
    const result = await RestauranteRepository.historicoEntregador({ page, limit: 15 })
    history.value = result.data
    historyPage.value = result.meta.page
...
function deliveryDuration(order: RestaurantePedido) {
  ...
  const end = order.Entrega?.entregueAt ? new Date(order.Entrega.entregueAt).getTime() : null
  return start && end && end >= start ? Math.round((end - start) / 60_000) : null
}
  ...
  <main class="driver-app">
    ...
    <template v-else>
      <header class="driver-header">
        ...
        <div class="header-actions"><button class="logout" aria-label="Histórico de entregas" @click="openHistory"><History :size="19" /></button><button class="logout" aria-label="Sair" @click="clearDriverSession"><LogOut :size="19" /></button></div>
      </header>
      <section class="driver-content">
        <template v-if="view === 'history'">
          <button class="back-offers" @click="openOffers"><ChevronRight :size="17" />Voltar às ofertas</button>
          <div class="section-title history-title"><div><p class="eyebrow">MINHA JORNADA</p><h1>Histórico de entregas</h1><small>Pedidos concluídos ou não concluídos por você.</small></div><button class="refresh" :disabled="historyLoading" @click="loadHistory(historyPage)"><RefreshCw :class="{ 'animate-spin': historyLoading }" :size="18" /></button></div>
          <div v-if="historyLoading && !history.length" class="loading"><RefreshCw class="animate-spin" />Carregando histórico...</div>
          <div v-else-if="!history.length" class="empty"><History :size="34" /><h2>Nenhuma entrega finalizada</h2><p>Quando você concluir sua primeira rota, ela aparecerá aqui.</p></div>
          <article v-for="order in history" v-else :key="order.id" class="history-card"><div class="history-top"><span class="order-code">{{ order.codigo }}</span><span class="history-status" :class="{ failed: order.entregaStatus === 'FALHOU' }">{{ order.entregaStatus === 'ENTREGUE' ? 'Entregue' : 'Não concluída' }}</span></div><h2>{{ order.clienteNomeSnapshot || 'Cliente' }}</h2><p><MapPin :size="16" />{{ address(order) }}</p><div class="history-meta"><span><Clock3 :size="15" />{{ deliveryDate(order) }}</span><span v-if="deliveryDuration(order) !== null"><CheckCircle2 :size="15" />{{ deliveryDuration(order) }} min</span><strong>{{ money(order.total) }}</strong></div></article>
          <div v-if="historyPages > 1" class="history-pagination"><button :disabled="historyLoading || historyPage <= 1" @click="loadHistory(historyPage - 1)">Anterior</button><span>Página {{ historyPage }} de {{ historyPages }}</span><button :disabled="historyLoading || historyPage >= historyPages" @click="loadHistory(historyPage + 1)">Próxima</button></div>
        </template>
        <template v-else>
        ...
        <template v-else>
          ...
          <div v-if="!context?.ofertas.length" class="empty justify-center items-center flex flex-col"><Clock3 :size="34" /><h2>Nenhuma oferta agora</h2><p>Deixe seu status online. Assim que a expedição liberar um pedido, ele aparecerá aqui.</p></div>
...
.header-actions{display:flex;align-items:center;gap:2px}.back-offers{display:flex;align-items:center;gap:4px;margin:0;padding:0;border:0;background:transparent;color:#08716b;font:inherit;font-size:12px;font-weight:800;cursor:pointer}.back-offers svg{transform:rotate(180deg)}.history-title small{display:block;margin-top:6px;color:#728196;font-size:12px}.history-card{margin-top:17px;padding:19px;border:1px solid #dbe3e8;border-radius:17px;background:#fff;box-shadow:0 5px 20px #1d293909}.history-top,.history-meta{display:flex;align-items:center;justify-content:space-between;gap:12px}.history-card h2{margin:11px 0 7px;font-size:18px}.history-card p{display:flex;gap:7px;margin:0;color:#637384;font-size:13px;line-height:1.4}.history-status{padding:5px 9px;border-radius:20px;background:#d5f5eb;color:#087460;font-size:10px;font-weight:900}.history-status.failed{background:#fff0ed;color:#c2553f}.history-meta{margin-top:15px;padding-top:12px;border-top:1px solid #e8edf0;color:#718091;font-size:11px}.history-meta span{display:flex;align-items:center;gap:4px}.history-meta strong{margin-left:auto;color:#1d2b3a;font-size:14px}.history-pagination{display:flex;align-items:center;justify-content:space-between;margin:20px 0;color:#718091;font-size:11px}.history-pagination button{border:1px solid #d6e1e6;border-radius:8px;background:#fff;padding:7px 10px;color:#08716b;font:inherit;font-weight:800;cursor:pointer}.history-pagination button:disabled{cursor:not-allowed;opacity:.5}@media(min-width:700px){.history-card{padding:22px}}
</style>
