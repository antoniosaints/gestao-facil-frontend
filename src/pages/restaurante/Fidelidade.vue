    ...
    <template v-else>
      <Card class="overflow-hidden border-primary/70 bg-gradient-to-br from-primary/10 via-background to-primary/10 dark:from-primary/30 dark:to-background">
        <CardContent class="flex flex-col gap-5 p-5 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div class="flex items-center gap-3 rounded-xl bg-background/80 px-4 py-3 shadow-sm"><Label for="fidelidade-ativa" class="font-medium">Ativar fidelidade</Label><Switch id="fidelidade-ativa" v-model="form.ativo" /></div>
        </CardContent>
      </Card>
      ...
      <div class="grid gap-5 lg:grid-cols-[1fr_.85fr]">
        <Card><CardHeader><CardTitle class="flex items-center gap-2 text-lg"><CheckCircle2 class="h-5 w-5 text-blue-600" />Regra para pontuar</CardTitle><CardDescription>O pedido concluído conta uma vez quando contiver pelo menos uma das escolhas abaixo.</CardDescription></CardHeader><CardContent class="space-y-6">
          ...
          <div class="space-y-2"><Label>Categorias elegíveis</Label><p class="text-xs text-muted-foreground">Deixe sem seleção para não limitar por categoria.</p><div class="grid gap-2 sm:grid-cols-2"><label v-for="category in categories" :key="category.id" class="flex cursor-pointer items-center gap-2 rounded-lg border p-3 text-sm hover:bg-muted/40"><Checkbox :model-value="form.categoriaIds.includes(category.id)" @update:model-value="form.categoriaIds = toggle(form.categoriaIds, category.id, Boolean($event))" />{{ category.nome }}</label></div></div>
          <div class="space-y-2"><Label>Produtos elegíveis</Label><p class="text-xs text-muted-foreground">Pode combinar produtos e categorias.</p><div class="grid max-h-64 gap-2 overflow-y-auto pr-1 sm:grid-cols-2"><label v-for="item in items" :key="item.id" class="flex cursor-pointer items-center gap-2 rounded-lg border p-3 text-sm hover:bg-muted/40"><Checkbox :model-value="form.catalogoItemIds.includes(item.id)" @update:model-value="form.catalogoItemIds = toggle(form.catalogoItemIds, item.id, Boolean($event))" /><span class="truncate">{{ item.nome }}</span></label></div></div>
        </CardContent></Card>
