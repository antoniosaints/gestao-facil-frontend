  ...
  <section class="space-y-4">
    <header class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <p class="text-sm text-muted-foreground">Produção por cozinha, bar, pizzaria ou outro setor
          produtor.</p>
      </div>
      <div class="flex gap-2"><Button variant="outline" :disabled="loading" @click="carregar(true)">
        ...
        </Button><Button v-if="canConfigure" variant="outline" @click="configModal = true">
    ...
    <div class="flex flex-col gap-2 rounded-xl border bg-card p-2 sm:flex-row sm:items-center">
      <Select v-model="pontoFiltro">
        ...
        <SelectContent>
          ...
          <SelectItem v-for="ponto in pontos.filter((item) => item.ativo)" :key="ponto.id" :value="String(ponto.id)">{{
    ...
    <div v-if="loading" class="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
      <Skeleton v-for="item in 8" :key="item" class="h-52 rounded-xl" />
    </div>
    <div v-else-if="!pontos.some((item) => item.ativo)" class="rounded-xl border border-dashed p-10 text-center">
      <Settings2 class="mx-auto mb-3 h-9 w-9 text-muted-foreground" />
      <p class="font-medium">Nenhum ponto de produção ativo</p>
      <p class="mb-4 text-sm text-muted-foreground">Associe categorias à cozinha, bar ou outro setor.</p><Button
        v-if="canConfigure" @click="editarPonto()">
    ...
    <div v-else class="grid grid-cols-1 items-start gap-3 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
      <Card v-for="ticket in filtrados" :key="ticket.id" class="overflow-hidden rounded-xl border-l-4"
            ...
            <div class="min-w-0">
              <div class="flex flex-wrap items-center gap-1.5">
                ...
                <Badge v-if="ticket.tipo !== 'INICIAL'" variant="destructive" class="text-[10px]">{{
                  typeLabel[ticket.tipo] }}</Badge>
              </div>
              <p class="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground"><span
                  class="flex items-center gap-1">
                  <Clock3 class="h-3.5 w-3.5" />{{ elapsed(ticket.createdAt) }}
                </span><span>{{ ticket.Pedido.Mesa?.nome || ticket.Pedido.origem.replace('_', ' ') }}</span></p>
            </div>
            <div class="flex shrink-0 flex-col items-end gap-1">
              <Badge variant="outline">{{ statusLabel[ticket.status] }}</Badge><span
                class="text-[11px] text-muted-foreground">{{ ticket.Ponto.nome }}</span>
            </div>
          </div>
        </CardHeader>
            ...
            <div v-for="item in ticket.itens" :key="item.PedidoItem.id" class="py-2">
              <div class="flex items-start gap-2"><span
                ...
                <div class="min-w-0 flex-1">
                  <p class="text-sm font-semibold leading-5">{{ item.PedidoItem.nomeSnapshot }}<span
                      v-if="item.PedidoItem.tamanhoSnapshot" class="font-normal text-muted-foreground"> · {{
                        item.PedidoItem.tamanhoSnapshot }}</span></p>
                  <p v-if="selections(item)" class="mt-0.5 text-xs leading-4 text-muted-foreground">+ {{
                    selections(item) }}</p>
                </div>
              </div>
              <p v-if="item.PedidoItem.observacao || item.observacao"
                class="mt-1.5 rounded-md bg-amber-500/10 px-2 py-1.5 text-xs font-medium leading-4 text-amber-800 dark:text-amber-200">
                {{ item.PedidoItem.observacao || item.observacao }}</p>
            </div>
          </div>
          <p v-if="ticket.Pedido.observacao" class="rounded-lg bg-muted px-2.5 py-2 text-xs leading-4">
            <strong>Observação do pedido:</strong> {{ ticket.Pedido.observacao }}
          </p>
        </CardContent>
        <CardFooter v-if="canOperate" class="gap-2 border-t px-3 py-2.5"><Button v-if="nextStatus[ticket.status]"
            size="sm" class="min-h-9 flex-1" @click="avancar(ticket)">
            <ChefHat class="mr-1.5 h-4 w-4" />{{ nextLabel[ticket.status] }}
          </Button><Button size="sm" variant="outline" class="min-h-9"
            :class="nextStatus[ticket.status] ? '' : 'w-full'" @click="reimprimir(ticket)">
          ...
          <div v-for="ponto in pontos" :key="ponto.id" class="flex items-center justify-between rounded-xl border p-3">
            <div>
              ...
              <p class="text-xs text-muted-foreground">
                {{(ponto.roteamentos.map((route) => route.Categoria.nome).join(',') || 'Sem categorias')}}
              </p>
            </div>
            <div class="flex items-center gap-2">
              <Badge variant="outline">{{ ponto.ativo ? 'Ativo' : 'Inativo' }}</Badge><Button size="sm"
                variant="outline" @click="editarPonto(ponto)">Editar</Button>
            </div>
          </div>
        </div>
        <DialogFooter><Button variant="outline" @click="configModal = false">Fechar</Button><Button
            @click="editarPonto()">
    ...
    <Dialog v-model:open="pontoModal">
      <DialogContent>
        ...
        <div class="grid gap-4 sm:grid-cols-2">
          ...
          <div class="space-y-2"><Label>Ordem</Label><Input v-model.number="form.ordem" type="number" /></div>
        ...
        <DialogFooter><Button variant="outline" @click="pontoModal = false">Cancelar</Button><Button
            :disabled="saving || !form.nome.trim()" @click="salvarPonto">Salvar ponto</Button></DialogFooter>
      </DialogContent>
    </Dialog>
  </section>
</template>
