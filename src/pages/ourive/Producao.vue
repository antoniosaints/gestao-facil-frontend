...
const orderStatusLabel = (status: string) => ({ RECEBIDA: 'Recebida', ORCAMENTO: 'Orçamento', PRODUCAO: 'Produção', REVISAO: 'Revisão' } as Record<string, string>)[status] || status
...
async function saveStage() {
  if (!form.nome || !form.responsavelIds.length || form.comissoes.some((item: any) => !item.usuarioId || !item.referencia)) return toast.info('Informe etapa, responsáveis e os valores de comissão.')
  try {
    await OuriveRepository.criarEtapa(selected.value.id, { ...form, prazoPrevisto: form.prazoPrevisto || undefined, responsavelIds: form.responsavelIds.map(Number), comissoes: form.comissoes.map((item: any) => ({ ...item, usuarioId: Number(item.usuarioId), referencia: Number(item.referencia) })) })
...
<template>
  <section class="space-y-5">
    ...
    <div class="grid gap-5 xl:grid-cols-[20rem_minmax(0,1fr)]">
      <Card class="h-fit xl:sticky xl:top-4"><CardHeader class="pb-3"><CardTitle class="text-base">Ordens em andamento</CardTitle><div class="relative mt-2"><Search class="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><Input v-model="search" class="pl-9" placeholder="Buscar OS ou cliente" /></div></CardHeader><CardContent class="max-h-[28rem] space-y-2 overflow-y-auto pb-4"><button v-for="order in filteredOrders" :key="order.id" class="w-full rounded-xl border p-3 text-left transition hover:bg-muted" :class="selected?.id === order.id ? 'border-primary bg-primary/5 ring-1 ring-primary/30' : ''" @click="openOrder(order.id)"><div class="flex items-start justify-between gap-2"><p class="font-semibold">{{ order.codigoRastreio }}</p><Badge :variant="order.status === 'REVISAO' ? 'secondary' : 'outline'">{{ orderStatusLabel(order.status) }}</Badge></div><p class="mt-1 truncate text-sm text-muted-foreground">{{ order.ordemServico?.Cliente?.nome || 'Cliente não informado' }}</p><p class="mt-1 line-clamp-2 text-xs text-muted-foreground">{{ order.ordemServico?.descricao || 'Sem solicitação' }}</p></button><p v-if="!filteredOrders.length" class="py-6 text-center text-sm text-muted-foreground">Nenhuma OS em andamento.</p></CardContent></Card>

      <div v-if="selected" class="space-y-5"><Card><CardContent class="p-5"><div class="flex flex-col justify-between gap-4 sm:flex-row sm:items-start"><div><div class="flex flex-wrap items-center gap-2"><p class="text-xl font-bold">{{ selected.codigoRastreio }}</p><Badge :variant="selected.status === 'REVISAO' ? 'secondary' : 'outline'">{{ selected.status === 'REVISAO' ? 'Em revisão' : 'Em produção' }}</Badge></div><p class="mt-1 font-medium">{{ selected.ordemServico?.Cliente?.nome || 'Cliente não informado' }}</p><p class="mt-1 text-sm text-muted-foreground">{{ selected.ordemServico?.descricao }}</p></div><Button variant="outline" size="sm" @click="router.push({ name: 'ourive-ordem', params: { id: selected.id } })"><ExternalLink class="mr-2 h-4 w-4" />Abrir OS</Button></div><div class="mt-5 grid gap-3 border-t pt-4 sm:grid-cols-2"><div class="flex items-center gap-2"><ListTodo class="h-4 w-4 text-primary" /><span class="text-sm">{{ stageCounts.completed }} de {{ stageCounts.total }} etapas aprovadas</span></div><div class="flex items-center gap-2"><UserRound class="h-4 w-4 text-primary" /><span class="text-sm">Acompanhe responsáveis e prazos por etapa</span></div></div></CardContent></Card>
