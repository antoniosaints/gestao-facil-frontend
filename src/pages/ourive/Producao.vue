  ...
  <section class="space-y-5">
    <div class="flex flex-col justify-between gap-3 md:flex-row md:items-end">
      ...
      <Button
        ...
        @click="showNewStage = !showNewStage"
        ><Plus class="mr-2 h-4 w-4" />{{
          showNewStage ? 'Fechar formulário' : 'Nova etapa'
        }}</Button
      >
    </div>
    ...
    <div class="grid gap-5 xl:grid-cols-[20rem_minmax(0,1fr)]">
      ...
      <div v-if="selected" class="space-y-5">
        ...
        <Card v-if="showNewStage && can('PRODUCAO')"
          ><CardHeader
            ><CardTitle class="text-base">Nova etapa para {{ selected.codigoRastreio }}</CardTitle
            ><CardDescription
              >Defina a atividade, responsáveis, prazo e comissão.</CardDescription
            ></CardHeader
          ><CardContent class="space-y-5"
            ><div class="grid gap-3 md:grid-cols-2">
              <Input v-model="form.nome" placeholder="Ex.: Ajuste de aro" /><select
              ...
              ><Calendarpicker v-model="form.prazoPrevisto" :teleport="true" /><Input
            ...
            <div class="rounded-xl border p-4">
              ...
              >
                ...
                ><select v-model="split.tipo" class="h-10 rounded-md border bg-background px-3">
                  ...
                  <option value="VALOR_FIXO">Valor fixo</option></select
                ><Input
