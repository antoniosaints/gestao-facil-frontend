...
function eventosDoDia(dia: Date | null) {
    ...
    return props.eventos.filter(e => format(new Date(e.startAt), "yyyy-MM-dd") === dataStr)
}
    ...
    <div>
        ...
        <div class="grid grid-cols-7 gap-1 text-xs">
            <div v-for="(dia, i) in days" :key="i" @click="navigateToDay(dia)"
                class="h-24 border rounded cursor-pointer p-2 text-left" :class="[
                    isSameMonth(dia, currentMonth)
                        ? 'bg-white dark:bg-gray-950'
                        : 'bg-gray-200 dark:bg-gray-800',
                    isSameDay(dia, hoje)
                        ? 'bg-gray-200 dark:bg-slate-900 !border-primary !border-2'
                        : ''
                ]">
