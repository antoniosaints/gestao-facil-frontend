...
const calendarioDescricao = computed(() => {
  if (props.mode === 'receitas')
    return 'Leitura por vencimento com foco em recebimentos pendentes e realizados.'
  if (props.mode === 'despesas')
    return 'Leitura por vencimento com foco em pagamentos pendentes e realizados.'
...
function navigateMonth(direction: 'prev' | 'next') {
  store.currentMonth =
    direction === 'prev' ? subMonths(store.currentMonth, 1) : addMonths(store.currentMonth, 1)
...
function applyPreset(
  preset: 'today' | 'due-today' | 'overdue' | 'receitas' | 'despesas' | 'reset',
) {
  if (preset === 'today') {
...
const filtrosAtivos = computed(() => {
  ...
  if (!tipoTravado.value && filtros.value.tipo !== 'TODOS')
    items.push(`Tipo: ${filtros.value.tipo}`)
  if (filtros.value.status !== 'TODOS') items.push(`Status: ${filtros.value.status}`)
  if (filtros.value.contaFinanceiraId !== 'all') {
    items.push(
      `Conta: ${getContaNomeById(filtros.value.contaFinanceiraId) || filtros.value.contaFinanceiraId}`,
    )
  }
...
const indicadores = computed(() => [
  ...
  },
  {
    titulo:
      props.mode === 'receitas'
        ? 'Recebível possível do dia'
        : props.mode === 'despesas'
          ? 'Pagável possível do dia'
          : 'Saldo possível do dia',
...
<template>
  <div class="max-w-7xl space-y-2 pb-8 md:pb-0">
    <div class="rounded-lg">
      <div
        class="flex flex-col gap-4 lg:flex-row lg:items-center justify-center lg:justify-between"
      >
        <div class="space-y-1">
          <h2 class="flex items-center gap-2 text-2xl font-bold text-foreground">
            <component
              :is="headerIcon"
              class="h-5 w-5 lg:h-6 lg:w-6 text-primary dark:text-blue-500"
            />
            {{ title }}
          </h2>
          <div
            v-if="filtrosAtivos.length"
            class="text-xs text-muted-foreground truncate hidden lg:block"
          >
        ...
        <div
          class="hidden lg:flex flex-wrap items-center justify-center lg:justify-between gap-2 self-center lg:self-auto"
        >
          <Button
            variant="outline"
            size="icon"
            class="hidden lg:inline-flex"
            v-tooltip="'Mês anterior'"
    ...
    <Tabs v-model="activeTab" default-value="geral" class="space-y-2">
        ...
        <div class="overflow-x-auto">
          <TabsList class="inline-flex w-full md:w-max justify-start gap-2 rounded-md border p-1">
            <TabsTrigger value="geral" class="px-4">
              <span class="flex items-center gap-2"> <BarChart class="h-4 w-4" />Geral </span>
            </TabsTrigger>
            ...
            <TabsTrigger value="resumo" class="px-4">
              <span class="flex items-center gap-2"> <FileText class="h-4 w-4" />Resumo </span>
            </TabsTrigger>
          </TabsList>
        </div>
      </div>
      ...
      <TabsContent value="geral" class="space-y-4">
        ...
        <div
          v-else-if="!displayedDias.length"
          ...
          <div v-for="dia in displayedDias" :key="String(dia.dia)" class="space-y-0">
            ...
            >
              ...
              <div class="flex flex-wrap items-center gap-2">
                ...
                >
                  <span class="hidden md:inline-flex">Realizado</span>
                  {{ formatCurrencyBR(dia.saldoRealizado) }}
                </Badge>
                <Badge
                ...
                >
                  <span class="hidden md:inline-flex">Previsto</span>
                  {{ formatCurrencyBR(dia.saldoPrevisto) }}
                </Badge>
                <Button
                  variant="outline"
                  size="icon"
                  class="h-8 w-8"
                  v-tooltip="'Adicionar lançamento'"
                    ...
                    <div class="flex items-center justify-between gap-3">
                      <div class="min-w-0">
                        ...
                        <p class="truncate text-[11px] text-muted-foreground">
                          ...
                          {{
                            item.numero === 0
                              ? 'Entrada'
                              : `${item.numero} de ${item.totalParcelas}`
                          }}
                          • Venc.
                          {{ format(new Date(item.vencimento), 'dd/MM/yyyy', { locale: ptBR }) }}
                          <span v-if="item.dataPagamento">
                            • Pgto
                            {{
                      ...
                      <div class="flex flex-col gap-1 text-right">
                        ...
                        <div class="flex flex-wrap justify-end gap-1">
                          <span
                            class="border shadow-none rounded-sm border-dashed px-2 py-0 text-[10px]"
                            :class="
                              item.status === 'PAGO'
                                ? 'bg-blue-100 text-blue-700 dark:bg-blue-950/40 dark:text-blue-300 border-blue-300 dark:border-blue-950'
                                : item.status === 'ATRASADO'
                                  ? 'bg-rose-100 text-rose-700 dark:bg-rose-950/40 dark:text-rose-300 border-rose-300 dark:border-rose-950'
                  ...
                  <div class="flex items-center gap-1" @click.stop>
                    <div class="hidden items-center gap-1 lg:flex">
                      <Button
                        ...
                        class="h-8 w-8"
                        v-tooltip="'Editar'"
                        :data-testid="`editar-parcela-${item.parcelaId}`"
                      ...
                      <Button
                        ...
                        v-tooltip="'Estornar'"
                        :data-testid="`estornar-parcela-${item.parcelaId}`"
                      ...
                      <Button
                        v-if="item.cobrancaLink"
                        variant="outline"
                        size="icon"
                        class="h-8 w-8"
                        v-tooltip="'Abrir cobrança'"
                        :data-testid="`abrir-cobranca-${item.parcelaId}`"
                        @click="openLinkCobranca(item.cobrancaLink)"
                      >
                        <ExternalLink class="h-4 w-4" />
                      </Button>
                      <Button
                        v-if="props.mode === 'receitas' && !item.pago"
                        variant="outline"
                        size="icon"
                        class="h-8 w-8"
                        v-tooltip="'Enviar cobrança pelo WhatsApp'"
                        :data-testid="`enviar-cobranca-${item.parcelaId}`"
      ...
      <TabsContent value="calendario" class="space-y-4">
        <FinanceiroCalendario
          v-model:selected-date="calendarioData"
          :eventos="eventosCalendario"
          :title="calendarioTitulo"
          :description="calendarioDescricao"
          @event-click="openEventoCalendario"
        />
      </TabsContent>
        ...
        <section class="grid gap-4 grid-cols-1 md:grid-cols-2 xl:grid-cols-3">
          <Card v-for="item in indicadores" :key="item.titulo" class="rounded-xl shadow transition">
            <CardHeader class="p-2 px-4">
              <CardTitle
                class="flex flex-row items-center gap-2 text-sm text-gray-700 dark:text-gray-300"
              >
            ...
            <CardContent class="space-y-1 px-4 pb-2">
              ...
              <p
                class="text-xs leading-relaxed text-muted-foreground truncate"
                :title="item.detalhe"
              >
                {{ item.detalhe }}
              </p>
      ...
      <div class="flex items-center gap-4">
        <span class="text-muted-foreground">
          ...
          <span class="font-semibold text-foreground">{{
            formatCurrencyBR(resumo.saldoInicialPeriodo)
          }}</span>
        </span>
        <span class="text-muted-foreground">
          Esperado
          <span class="font-semibold text-foreground">{{
            formatCurrencyBR(resumo.saldoPossivelDia)
          }}</span>
        </span>
      </div>
    </div>
    ...
    <MobileBottomBar v-if="uiStore.isMobile">
      ...
      >
        <span
          class="flex h-12 w-12 items-center justify-center rounded-full bg-primary text-white shadow-lg dark:text-white"
        >
    ...
    <ModalView v-if="!tipoTravado" v-model:open="openModalLancar" title="Novo lançamento" size="sm">
      <div class="grid grid-cols-2 gap-4 p-4">
        ...
        <button
          class="rounded-2xl border bg-rose-50 p-4 text-left transition hover:bg-rose-100 dark:bg-rose-950/30"
    ...
    <ModalView
      v-model:open="openModalEvento"
      :title="eventoSelecionado?.descricao || 'Resumo da parcela'"
      size="lg"
    >
      <div v-if="eventoSelecionado" class="space-y-4 p-4 -mt-6">
        <div class="flex flex-wrap gap-2 -mb-2">
          <span
            class="border border-dashed border-muted shadow-none px-2 rounded-sm text-sm"
            :class="
        ...
        >
          ...
          <div class="text-right">
            <p class="text-sm font-medium text-foreground">
              {{
                eventoSelecionado.numero === 0
                  ? 'Entrada'
                  : `Parcela ${eventoSelecionado.numero} de ${eventoSelecionado.totalParcelas}`
              }}
            </p>
          ...
          <div class="flex items-start gap-2 rounded-lg border p-3">
            ...
            <div class="min-w-0">
              ...
              <p class="truncate text-sm text-foreground">
                {{ eventoSelecionado.categoria || 'Não informado' }}
              </p>
            </div>
          </div>
          <div class="flex items-start gap-2 rounded-lg border p-3">
            ...
            <div class="min-w-0">
              ...
              <p class="truncate text-sm text-foreground">
                {{
                  [eventoSelecionado.conta, eventoSelecionado.cliente]
                    .filter(Boolean)
                    .join(' • ') || 'Não informado'
                }}
              </p>
            ...
            <div class="min-w-0">
              ...
              <p class="text-sm text-foreground">
                <span v-if="eventoSelecionado.dataPagamento">
                  {{
                    format(new Date(eventoSelecionado.dataPagamento), 'dd/MM/yyyy', {
                      locale: ptBR,
                    })
                  }}
                </span>
                <span v-else>Pendente</span>
                <span v-if="eventoSelecionado.formaPagamento">
                  • {{ eventoSelecionado.formaPagamento }}</span
                >
              </p>
            </div>
          </div>
        ...
        <div class="space-y-2 border-t pt-3">
          ...
          <div class="flex flex-wrap gap-2">
            <Button
              v-if="!eventoSelecionado.pago"
              class="min-w-[9rem] flex-1"
              @click="handleEfetivarFromModal(eventoSelecionado)"
            >
              <CheckCircle2 class="h-4 w-4" />
              {{ eventoSelecionado.tipo === 'DESPESA' ? 'Pagar' : 'Receber' }}
            </Button>
            <Button
              v-else
              class="min-w-[9rem] flex-1 bg-warning text-white hover:bg-warning/80"
            ...
            <Button
              v-if="
                uiStore.canCreateCharge &&
                !eventoSelecionado.pago &&
                eventoSelecionado.tipo === 'RECEITA' &&
                !eventoSelecionado.cobrancaLink
              "
            ...
            <Button
              v-if="props.mode === 'receitas' && !eventoSelecionado.pago"
              variant="outline"
              class="min-w-[9rem] flex-1"
              @click="abrirCobrancaRapida(eventoSelecionado)"
            >
              <Send class="h-4 w-4" /> Enviar pelo WhatsApp
            </Button>
            <Button
              v-if="eventoSelecionado.cobrancaLink"
              variant="outline"
              class="min-w-[9rem] flex-1"
          ...
          <div class="flex flex-wrap gap-2">
            <RouterLink
              :to="`/financeiro/detalhes?id=${eventoSelecionado.id}`"
            ...
            <Button
              variant="outline"
              class="min-w-[8rem] flex-1"
              @click="handleEditarFromModal(eventoSelecionado)"
            >
    ...
    <ModalView v-model:open="openModalFiltros" title="Filtros do fluxo financeiro" size="lg">
      <div class="grid gap-4 p-4 md:grid-cols-2">
        <div class="space-y-2 md:col-span-2">
          ...
          <div class="flex flex-wrap gap-2">
            <Button type="button" variant="outline" size="sm" @click="applyPreset('today')"
              >Hoje</Button
            >
            <Button type="button" variant="outline" size="sm" @click="applyPreset('due-today')"
              >Vencendo hoje</Button
            >
            <Button type="button" variant="outline" size="sm" @click="applyPreset('overdue')"
              >Atrasados</Button
            >
            <Button
              v-if="!tipoTravado"
        ...
        <div class="space-y-2 md:col-span-2">
          ...
          <div class="relative">
            ...
            <Input
              v-model="filtros.search"
              class="pl-9"
              placeholder="Descrição, UID, categoria ou cliente"
            />
          </div>
        </div>
        ...
        <div class="flex justify-end gap-2 md:col-span-2">
          ...
          <Button @click="applyFilters"> <Filter class="h-4 w-4" /> Aplicar filtros </Button>
        </div>
      </div>
    </ModalView>
