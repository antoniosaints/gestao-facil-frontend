...
<template>
    <div class="space-y-4">
        <div class="flex justify-between items-center mb-2 bg-background border px-4 rounded-md py-4">
        ...
        <div v-else class="space-y-4">
            <Card v-for="dia in lancamentos" :key="dia.dia" class="bg-transparent border-none shadow-none">
                <CardContent class="p-0 border-none">
                    <p class="text-sm px-3">{{ format(new Date(dia.dia), "dd/MM/yyyy") }}</p>
                    <div class="flex flex-col gap-2">
                        <div v-for="item in dia.lancamentos" :key="item.id"
                            ...
                            <div v-if="item.tipo === 'RECEITA'"
                                class="absolute left-0 top-0 w-4 h-full rounded-l-md bg-success/90"></div>
                            <div v-else class="absolute left-0 top-0 w-4 h-full rounded-l-md bg-danger/90"></div>
                            <div>
