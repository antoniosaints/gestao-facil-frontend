...
<template>
  <div>
    ...
    <Dialog :open="!!selected" @update:open="!$event && (selected = null)"><DialogContent v-if="selected" class="max-h-[90vh] max-w-3xl overflow-y-auto"><DialogHeader><DialogTitle>{{ selected.Uid }} · {{ selected.nomeSnapshot }}</DialogTitle></DialogHeader>
      ...
      <div class="rounded-lg border"><div v-for="item in selected.itens" :key="item.id" class="flex justify-between border-b p-3 last:border-0"><span>{{ item.quantidade }}× {{ item.produtoNomeSnapshot }} / {{ item.varianteNomeSnapshot }}</span><b>{{ formatCurrencyBR(Number(item.subtotal)) }}</b></div><div v-for="combo in selected.comboSaidas || []" :key="`combo-${combo.id}`" class="flex justify-between gap-3 border-b p-3"><div><span>{{ combo.quantidade }}× {{ combo.nomeSnapshot }}</span><p v-for="component in combo.componentes" :key="component.id" class="text-xs text-muted-foreground">{{ component.quantidadePorCombo }}× {{ component.nomeSnapshot }}</p></div><b>{{ formatCurrencyBR(Number(combo.subtotal)) }}</b></div><div class="flex justify-between bg-muted/40 p-3 text-lg font-bold"><span>Total</span><span>{{ formatCurrencyBR(Number(selected.total)) }}</span></div></div>
