...
const loading = ref(false); const submitting = ref(false); const message = ref(''); const account = ref<any>(null)
...
async function submit() {
  ...
  } catch (error: any) { toast.error(error?.response?.data?.message || error?.message || 'Não foi possível continuar.') } finally { submitting.value = false }
}
...
watch(mode, async (currentMode) => {
  ...
  if (currentMode === 'account') {
    ...
    if (!token) {
      loading.value = false
      return router.push(`/lojas/${slug}/login`)
    }
    try {
      account.value = await LojaRepository.customerMe(slug, token)
    } catch {
...
<template>
  <div class="store-customer-page bg-white" :style="brandStyle">
    <!-- Conta do cliente -->
    <div v-if="mode === 'account'" class="min-h-screen bg-slate-50">
      ...
      <main class="flex items-center justify-center bg-white px-6 py-10">
        <div class="w-full max-w-md">
          ...
          <form v-else class="space-y-4" @submit.prevent="submit">
            ...
            <Button class="h-12 w-full rounded-xl text-base text-white" :style="{ backgroundColor: 'var(--shop-primary)' }" :disabled="submitting">
