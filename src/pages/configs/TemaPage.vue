    ...
    <form @submit.prevent="salvarTema">
        ...
        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            ...
            <CardDescription class="mt-1 max-w-2xl text-pretty">
              Personalize as cores principais nos modos claro e escuro. Contrastes, textos, cartões,
              bordas e estados suaves são calculados automaticamente.
            </CardDescription>
          </div>
          <div class="flex flex-wrap gap-2">
            <Button
              type="button"
              variant="outline"
              class="min-h-10 active:scale-[0.96]"
              @click="restaurarSalvo"
            >
      ...
      <CardContent class="space-y-7 p-5 lg:p-6">
        <section class="space-y-3">
          <div>
            ...
            <p class="text-sm text-muted-foreground">
              Escolha um ponto de partida e ajuste cada cor se desejar.
            </p>
          </div>
          <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            ...
            >
              <span class="flex -space-x-2">
                <i
                  v-for="cor in [
                    preset.tema.primariaLight,
                    preset.tema.sidebarLight,
                    preset.tema.fundoLight,
                  ]"
                  :key="cor"
        ...
        <div class="grid gap-6 xl:grid-cols-2">
          <section
            v-for="grupo in colorGroups"
            :key="grupo.mode"
            class="rounded-2xl bg-card p-2 shadow-[var(--shadow-theme-card)]"
          >
            <div class="rounded-xl bg-background p-4">
              ...
              <div class="grid gap-4 sm:grid-cols-3">
                <label v-for="field in grupo.fields" :key="field.key" class="space-y-2">
                  ...
                  <span
                    class="flex min-h-11 items-center gap-2 rounded-lg border bg-card p-1.5 focus-within:ring-2 focus-within:ring-ring"
                  >
                    <input
                  ...
                  <small class="block text-pretty text-[11px] leading-4 text-muted-foreground">{{
                    field.help
                  }}</small>
                </label>
              </div>
        ...
        <section class="space-y-4">
          ...
          <div class="grid gap-3 lg:grid-cols-2">
            <button
              v-for="style in navigationStyles"
              :key="style.value"
              type="button"
              class="flex min-h-32 items-start gap-4 rounded-2xl border bg-card p-4 text-left transition active:scale-[0.98]"
              :class="
                estiloUi === style.value
                  ? 'border-primary ring-2 ring-primary/30'
                  : 'hover:bg-muted/40'
              "
              @click="estiloUi = style.value"
            >
              <span
              ...
              <span class="min-w-0 flex-1">
                <span class="flex items-center justify-between gap-3">
                  <strong class="text-sm">{{ style.title }}</strong>
                  <Check v-if="estiloUi === style.value" class="h-5 w-5 shrink-0 text-primary" />
                </span>
                <small class="mt-1 block text-sm leading-5 text-muted-foreground">{{
                  style.description
                }}</small>
              </span>
            </button>
            ...
            <div class="space-y-2 rounded-2xl bg-card p-4 shadow-[var(--shadow-theme-card)]">
              ...
              <div class="grid grid-cols-3 gap-2">
                <button
                  ...
                  :class="
                    form.radius === raio.value
                      ? 'border-primary ring-2 ring-primary/30'
                      : 'hover:bg-muted/40'
                  "
                >
                  <span
                    class="h-8 w-full border border-primary/40 bg-primary/15"
                    :style="{ borderRadius: raio.value }"
                  />
              ...
              <div class="flex min-h-64">
                <aside class="w-32 shrink-0 p-3" :style="previewSidebarStyle(mode)">
                  <div class="mb-5 flex items-center gap-2">
                    <span
                      class="grid h-8 w-8 place-items-center rounded-lg bg-black/10 text-xs font-black"
                      >GF</span
                    >
                ...
                <div class="min-w-0 flex-1 p-4">
                  <div class="mb-4 flex items-center justify-between">
                    ...
                    <span
                      ...
                      :style="previewPrimaryStyle(mode)"
                      >Nova venda</span
                    >
                  </div>
                  <div class="grid grid-cols-2 gap-3">
                    <div
                      v-for="(label, index) in ['Vendas hoje', 'Ticket médio']"
                      :key="label"
                      class="rounded-xl p-3"
                      :style="previewCardStyle(mode)"
                    >
                      <p class="text-[11px] opacity-60">{{ label }}</p>
                      <strong class="mt-2 block font-mono text-base tabular-nums">{{
                        index ? 'R$ 86,40' : '24'
                      }}</strong>
                    </div>
                  </div>
                  <div class="mt-3 rounded-xl p-3" :style="previewCardStyle(mode)">
                    ...
                    <div class="h-2 overflow-hidden rounded-full bg-black/10">
                      <div
                        class="h-full w-[72%] rounded-full"
      ...
      <CardFooter
        class="flex-col gap-3 border-t bg-card/70 px-5 py-4 sm:flex-row sm:justify-between"
      >
        <p class="flex items-center gap-2 text-xs text-muted-foreground">
          <Check class="h-4 w-4 text-success" /> Contraste de texto ajustado automaticamente
          conforme WCAG.
        </p>
        <Button
          type="submit"
          class="min-h-10 text-primary-foreground active:scale-[0.96]"
          :disabled="loading"
        >
...
const navigationStyles: Array<{
...
}> = [
  {
    value: 'PADRAO',
    title: 'Padrão',
    description: 'Mantém a barra lateral e o acesso direto às telas, como o sistema funciona hoje.',
    icon: PanelLeft,
  },
  {
    value: 'CARDS',
...
const colorGroups = [
  {
    mode: 'light',
    title: 'Modo claro',
    description: 'Superfícies luminosas e nítidas.',
    badge: 'LIGHT',
    icon: Sun,
    fields: [
      {
        key: 'primariaLight' as ThemeField,
        label: 'Cor primária',
        help: 'Botões, links, foco e destaques.',
      },
      {
        key: 'sidebarLight' as ThemeField,
        label: 'Sidebar',
        help: 'Navegação lateral no modo claro.',
      },
      {
        key: 'fundoLight' as ThemeField,
        label: 'Fundo',
        help: 'Base usada para derivar cartões e bordas.',
      },
    ],
  },
  {
    mode: 'dark',
    title: 'Modo escuro',
    description: 'Contraste confortável em baixa luz.',
    badge: 'DARK',
    icon: Moon,
    fields: [
      {
        key: 'primariaDark' as ThemeField,
        label: 'Cor primária',
        help: 'Destaques com luminosidade controlada.',
      },
      {
        key: 'sidebarDark' as ThemeField,
        label: 'Sidebar',
        help: 'Navegação lateral no modo escuro.',
      },
      {
        key: 'fundoDark' as ThemeField,
        label: 'Fundo',
        help: 'Base escura para superfícies e modais.',
      },
    ],
  },
]
...
const previewContainerStyle = (mode: ThemeMode) => ({
  backgroundColor: palette(mode).background,
  color: palette(mode).foreground,
})
const previewSidebarStyle = (mode: ThemeMode) => ({
  backgroundColor: palette(mode).sidebar,
  color: palette(mode).sidebarForeground,
})
const previewPrimaryStyle = (mode: ThemeMode) => ({
  backgroundColor: palette(mode).primary,
  color: palette(mode).primaryForeground,
})
const previewCardStyle = (mode: ThemeMode) => ({
  backgroundColor: palette(mode).card,
  boxShadow: `0 0 0 1px ${palette(mode).border}`,
})
...
async function salvarTema() {
  try {
    ...
    await ContaRepository.parametros({ temaPersonalizado: normalized, estiloUi: estiloUi.value })
    savedTheme.value = { ...normalized }
    uiStore.estiloUi = estiloUi.value
...
:global(:root) {
  --shadow-theme-card:
    0 0 0 1px rgb(15 23 42 / 0.06), 0 1px 2px -1px rgb(15 23 42 / 0.06),
    0 5px 16px rgb(15 23 42 / 0.04);
