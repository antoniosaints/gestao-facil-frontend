...
function aplicarPrecificacao(payload: {
  ...
  store.form.precoCompra = payload.custoCompra
  store.form.custoMedioProducao = payload.custoProducao
  store.form.preco = payload.precoVenda
  clearError('preco')
  secPrecos.value = true
  toast.success('Preço sugerido aplicado ao produto')
}
...
function buildPayload() {
  return {
    ...
    precoPromocional: isBlank(store.form.precoPromocional)
      ? null
      : formatToNumberValue(store.form.precoPromocional),
    precoCompra: isBlank(store.form.precoCompra)
      ? undefined
      : formatToNumberValue(store.form.precoCompra),
    ...
    ean: store.form.ean?.toString().trim() || null,
    tipoItem: store.form.tipoItem?.toString().trim() || '00',
    icmsCsosn: store.form.icmsCsosn?.toString().trim() || null,
    icmsCst: store.form.icmsCst?.toString().trim() || null,
    icmsModBcSt: store.form.icmsModBcSt?.toString().trim() || null, icmsMva: store.form.icmsMva ?? null, icmsReducaoBcSt: store.form.icmsReducaoBcSt ?? null, icmsAliquotaSt: store.form.icmsAliquotaSt ?? null,
    fcpAliquota: store.form.fcpAliquota ?? null, fcpStAliquota: store.form.fcpStAliquota ?? null, icmsDesoneradoValor: store.form.icmsDesoneradoValor ?? null, icmsDesoneradoMotivo: store.form.icmsDesoneradoMotivo?.toString().trim() || null,
    icmsCreditoAliquota: store.form.icmsCreditoAliquota ?? null, ipiCst: store.form.ipiCst?.toString().trim() || null, ipiCodigoEnquadramento: store.form.ipiCodigoEnquadramento?.toString().trim() || null, difalAliquotaInterna: store.form.difalAliquotaInterna ?? null, difalFcpAliquota: store.form.difalFcpAliquota ?? null,
    pisCst: store.form.pisCst?.toString().trim() || null,
    cofinsCst: store.form.cofinsCst?.toString().trim() || null,
  ...
  <div>
    <ModalView v-model:open="store.openModal" :title="title" :description="description" size="5xl">
        ...
        <div class="grid grid-cols-1 gap-4 md:grid-cols-12">
          <div class="md:col-span-5">
            ...
            <Input
              ...
              type="text"
              placeholder="Ex: Padrão, Azul G, 1L"
              :class="[
                'bg-background dark:bg-background/60',
                errors.nomeVariante && 'border-danger',
              ]"
          ...
          <div class="md:col-span-4">
            <label class="mb-1.5 block text-sm font-medium text-foreground">
              ...
              <span class="text-red-500"
                >* <span class="text-xs" v-if="estoqueReadonly">Não editável.</span></span
              >
            </label>
            <Input
              ...
              :class="[
                'bg-background read-only:cursor-not-allowed read-only:bg-muted disabled:cursor-not-allowed disabled:bg-muted dark:bg-background/70',
          ...
          <div class="grid grid-cols-1 gap-4 md:grid-cols-12">
            ...
            <div class="md:col-span-4">
              ...
              <p v-else class="mt-1 text-xs text-muted-foreground">
                Ativa a seção "Promoções" na loja online.
              </p>
            </div>
            ...
            <div>
              <div class="mb-1.5 flex items-center justify-between gap-2">
                ...
                <IaTextAssistant
                  ...
                  title="Descrição do produto com IA"
      ...
      <CalculadoraPrecificacao
        ...
        :preco-compra="store.form.precoCompra"
        :custo-producao="store.form.custoMedioProducao"
