        ...
        <div v-for="(item, index) in menu" :key="index" class="space-y-4">
            ...
            <div v-else-if="item.children && item.children.length" v-show="item.show !== false"
                ...
                <button @click="toggleDropdown(item.nome)"
                    class="dropdown-toggle w-full flex items-center bg-teal-900/30 justify-between px-4 py-2 text-left cursor-pointer rounded-xl"
                    :class="{ 'rounded-b-none': openDropdowns.includes(item.nome) }">
                    <div class="flex items-center space-x-3">
                        <i v-if="typeof item.icone === 'string'"
                            :class="[item.icone, 'text-teal-800 dark:text-teal-400']"></i>
                ...
                <div v-show="openDropdowns.includes(item.nome)"
                    class="dropdown-content flex flex-col p-1 gap-1 bg-teal-900/75 rounded-b-xl">
                    <router-link v-for="(child, i) in item.children" :key="i" :to="child.link || 'javascript:void(0)'"
                        v-show="child.show !== false" :class="[
                            'flex items-center p-2 text-gray-200 dark:text-gray-300 pl-4 hover:bg-teal-700/80 cursor-pointer rounded-xl',
                            route.path === child.link && 'bg-teal-700 dark:bg-teal-950'
                        ]">
                        <i v-if="typeof child.icone === 'string'"
                            :class="[child.icone, 'mr-2', 'text-teal-600 dark:text-teal-600']"></i>
            ...
            <!-- Link normal -->
            <router-link
                class="cursor-pointer dark:border-gray-500 mt-2 hover:bg-teal-900/50 text-gray-200 dark:text-gray-300 p-2 rounded-xl flex items-center"
                :class="[route.path === item.link ? 'bg-teal-900/80' : 'bg-teal-900/30']" v-else
