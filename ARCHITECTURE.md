...
- A página `/whatsapp` (app `whatsapp`) concentra a gestão de instâncias W-API: conexão por QR Code/pairing code, ações da instância e configuração de webhooks. Tokens nunca devem ser exibidos; a UI deve mostrar apenas que o token está configurado. Webhooks devem ser configurados por modal de confirmação, com URLs preenchidas a partir do backend e feedback por callback enviado à W-API.
- A página `/atendimento` (app `atendimento`, `pages/atendimento/AtendimentoPage.vue`) é a central operacional do canal: listagem de conversas, leitura/envio de mensagens, vínculo com cliente e status de atendimento em tempo real. Reaproveita o mesmo backend `/api/whatsapp` de conversas/mensagens (isolado por `contaId`) e consome os eventos Socket.IO `whatsapp:conversa:updated` e `whatsapp:mensagem:created`. Uma conversa pode ser iniciada a partir de um cliente via `POST /api/whatsapp/conversas/iniciar` (diálogo "Nova" ou parâmetro de rota `?cliente=<id>`); a ficha do cliente só deve oferecer esse atalho quando o app `atendimento` estiver ativo e o cliente tiver telefone/WhatsApp. Cada instância é uma caixa de atendimento: o seletor de caixas filtra a lista por `instanciaId` (`GET /api/whatsapp/conversas?instanciaId=`), mantendo o mesmo isolamento por `contaId`. O ciclo de vida da conversa é ESPERA (PENDENTE) → ABERTA → FINALIZADA: uma nova mensagem do cliente entra em ESPERA e só passa a ABERTA quando alguém assume via "Atender" (`POST /api/whatsapp/conversas/:id/atender`, que grava `atendenteId`) ou responde; a UI bloqueia o envio enquanto a conversa não estiver ABERTA para manter os KPIs de atendimento confiáveis. O menu `atendimento` é um pai com submenus (Painel, Chat, Agentes, Relatórios); apenas o Chat está implementado, os demais são placeholders. O chat oferece ações rápidas no composer (abertas pelo ícone Blocks ou por `/`) para enviar dados do sistema ao cliente; a primeira ferramenta é **Venda**, que exige contato vinculado a um cliente e restringe as vendas às do cliente vinculado (backend valida ambos em `whatsAppService.listConversationSales`/`sendConversationSale`).
...
- A aparência também controla `ParametrosConta.estiloUi`, uma preferência compartilhada por conta. `PADRAO` mantém a navegação por sidebar; `CARDS` usa `pages/navegacao/NavigationHome.vue` e `CardsNavigationHub.vue` para derivar módulos e submenus do mesmo `sidebarMenuOptions` já filtrado por permissões, apps e capacidades. As rotas operacionais continuam no layout principal, porém o layout oculta a sidebar nesse modo.
